...
int main() {
	...
	int range1[4] = { 0,30,40,70 }; //stores first value for each range
	int range2[4] = { 29,39,69,100 }; //stores last value for each range
	const int cats = 4;
...
int convert(string a) {
	int x;
	if (a.length() < 3) { //2 digit number check for letters
		if (!isdigit(a[1]))
